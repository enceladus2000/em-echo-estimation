...
def EMMC(src_signal, mic_signals, setup: EMMC_Setup, init_est: EMMC_Estimate) -> Generator[EMMC_Estimate, None, None]:
	assert (
...
def M_gridsearch(xmr, src_signal, setup: EMMC_Setup, calc_gain=True):
	...
	for pidx, phicand in enumerate(setup.phi_grid):
		...
		for m in range(setup.num_mics):
			Dxmsum += delay_signal(
				xmr[m], -tdoacand[m], setup.fdflen, setup.fdfwindow
			)
